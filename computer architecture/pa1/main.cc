...
int main(int argc, char* argv[]){
	...
	srd=32;}
		...
		if(opcode=="0110111"){//lui
			...
			X[srd]=(maxread-1)*4+0x10000000+imm*4096;
		...
		X[srd]=location+4;
		input.seekg(imm-4+X[srs1], std::ios::beg);
        }
		...
		if(func3=="000"){//beq
                        if(X[srd]==X[srs1]) input.seekg(imm-4, std::ios::cur);}
                        ...
                        if((unsigned int)X[srd]<(unsigned int)X[srs1]) input.seekg(imm-4, std::ios::cur);}
		...
		int imm=tosign(2048,func7+rs2);
			...
			if(X[srs1]-0x10000000+imm==0x20000000){
                        cin>>X[srd];}
                        else
		...
		if(func3=="000"){//sb
			...
			*(((short*)(char*)pdata+(X[srs1]-0x10000000+imm)))=X[srs2]%65536;}
			...
			if(X[srs1]-0x10000000+imm==0x20000000) cout<<X[srs2];
			else
