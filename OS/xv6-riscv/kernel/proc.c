...
#include "proc.h"
#include "defs.h"
int weight[40]={88761,71755,56483,46273,36291,29154,23254,18705,14949,11916,9548,7620,6100,2984,3906, 3121,2501,1991,1586,1277,1024,828,655,526,423,335,272,215,172,137,110,87,70,56,45,36,29,23,18,15};
int V=0, W=0, v0=0;
...
scheduler(void)
{
  ...
  for(;;){
    ...
    for(p = proc; p < &proc[NPROC]; p++) {
      acquire(&p->lock);
      if(p->state == RUNNABLE&&V>=(p->vruntime-v0)*W) {
        if(select==0||p->vdeadline<select->vdeadline)
		select=p;
      }
      release(&p->lock);
...
printf("name         pid       state            priority   runtime/weight   runtime      vruntime      vdeadline     is_eligible     tick %d\n",t);
  for(p = proc; p < &proc[NPROC]; p++) {
    ...
    if(p->pid==pid||!pid){
      ...
      printf("%d         %ld                %ld         %ld         %ld         ",p->nice,p->runtime/weight[p->nice],p->runtime,p->vruntime,p->vdeadline);
if(V>=(p->vruntime-v0)*W){
printf("true\n");
}
else
printf("false\n");
